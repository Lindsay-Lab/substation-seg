...
image_transform = transforms.Compose([
    transforms.Resize((512,512),transforms.InterpolationMode.BICUBIC, antialias=True)
])
...
downsample = transforms.Resize((256,256),transforms.InterpolationMode.BILINEAR, antialias=True) # for downsampling output mask in train loop
...
#         image = image[[3,2,1],:,:]
        ...
        if self.img_transforms:
            image = self.img_transforms(image) #image -> (512x512)
        if self.mask_transforms:
            mask = self.mask_transforms(mask) #mask -> (256x256)
...
for e in range(starting_epoch,starting_epoch+N_EPOCHS):    
    ...
    for batch in train_dataloader:
        ...
        output = model(data)
        output = downsample(output) #(512x512->256x256)
        if loss_type == 'FOCAL':
